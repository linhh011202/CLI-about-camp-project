...
public class CampCommitteeSlotChecker implements IGetCampSlots
{
    ...
    public CampCommitteeSlotChecker(CampDataBase campDataBase){this.campDataBase=campDataBase;}

    public int getCampSlots(String campName)
    {
        ArrayList<Camp> allCamps=campDataBase.getAllCamps();
        //Search camps for campName to check and return slots. If no slots, return error code. If no camp, return error code.
        for(int i=0;i<allCamps.size();++i)
        {
            if(allCamps.get(i).getCampName().equals(campName))
            {
                ...
                else return allCamps.get(i).getAvailableCampComSlots();
            }
        }
