...
public class ClashWithRegisteredChecker implements ICheckNoClash
{
    ...
    {
        if(campsRegistered==null)return true;
        ...
        //Should never be able to happen, but just in case unable to find camp to be registered in campDB
        if(campToRegisterEnd==null || campToRegisterEnd==null)return false;
        ...
        for(int i=0;i<campsRegistered.size();++i)
        {
            for(int j=0;j<allCamps.size();++j)
            {
                ...
                {
                    if(allCamps.get(j).getStartDate().isAfter(campToRegisterEnd) || campToRegisterStart.isAfter(allCamps.get(j).getEndDate()))
                    {
                        //No overlap;
                    }
                    else return false;
