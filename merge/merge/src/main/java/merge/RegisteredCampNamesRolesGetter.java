...
public class RegisteredCampNamesRolesGetter implements IGetCampNamesRolesRegistered
{
    ...
    public ArrayList<ArrayList<String>> getRegisteredCampNamesRoles(String studentName)
    {
        ...
        {
            //Find all entries that student is registered in, add the campName, and corresponding role to arrayList.
            if(allRegistration.get(i).getStudentName().equals(studentName) && !allRegistration.get(i).getDeregistered())
            {
                registeredCampsRoles.add(new ArrayList<String>(Arrays.asList(allRegistration.get(i).getCampName(), allRegistration.get(i).getRole())));
            }
        }
